...
class AdminTable:
    """
    Database access layer for the `admins` table.

    Implements create/get/get_by_email/update/list operations aligned with
    the migration in `db-service/migrations/100001_create-admin-schema.py`.
    """
    ...
    async def create(self, admin_create: AdminCreate) -> Optional[AdminInDB]:
        """Insert a new admin and return the created record (including hash)."""
        self.ready()
        ...
        try:
            ...
            async with self.database.pool.acquire() as connection:
                async with connection.transaction():
                    ...
                    props = admin_create.properties or {}
                    ...
                    record = await connection.fetchrow(
                        """
                        INSERT INTO admins
                            (email, password_hash, first_name, last_name, is_active, is_superadmin, platform_id, properties)
                        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                        RETURNING id, created_at, updated_at, email, first_name, last_name, is_active, is_superadmin, platform_id, properties, password_hash
                        """,
                        admin_create.email,
                        password_hash,
                        admin_create.first_name,
                        admin_create.last_name,
                        admin_create.is_active,
                        admin_create.is_superadmin,
                        admin_create.platform_id,
                        props_param,
                    )
            ...
            self.logger.error(f"{__name__}: Error inserting admin record - {e}")
            raise e

    async def get(self, id: str) -> Optional[AdminRead]:
        """Retrieve admin by id (does not include password hash)."""
        self.ready()
        ...
        try:
            async with self.database.pool.acquire() as connection:
                record = await connection.fetchrow(
                    """
                    SELECT id, created_at, updated_at, email, first_name, last_name, is_active, is_superadmin, platform_id, properties
                    FROM admins
                    WHERE id = $1
                    LIMIT 1
                    """,
            ...
            self.logger.error(f"{__name__}: Error retrieving admin by ID {id} - {e}")
            raise e

    async def get_by_email(self, email: str) -> Optional[AdminInDB]:
        """Retrieve admin by email (includes password hash for authentication)."""
        self.ready()
    ...
    async def update(self, id: str, admin_update: AdminUpdate) -> Optional[AdminRead]:
        """Apply partial updates to an admin and return the updated record."""
        self.ready()
        ...
        try:
            async with self.database.pool.acquire() as connection:
                async with connection.transaction():
                    ...
                    if admin_update.first_name is not None:
                        ...
                        values.append(admin_update.first_name)
                        idx += 1

                    if admin_update.last_name is not None:
                        set_clauses.append(f"last_name = ${idx}")
                        values.append(admin_update.last_name)
                        idx += 1

                    if admin_update.is_active is not None:
                        set_clauses.append(f"is_active = ${idx}")
                        values.append(admin_update.is_active)
                        idx += 1

                    if admin_update.is_superadmin is not None:
                        set_clauses.append(f"is_superadmin = ${idx}")
                        values.append(admin_update.is_superadmin)
                        idx += 1

                    if admin_update.platform_id is not None:
                        set_clauses.append(f"platform_id = ${idx}")
                        values.append(admin_update.platform_id)
                        idx += 1

                    if admin_update.properties is not None:
                        set_clauses.append(f"properties = ${idx}")
                        props_val = admin_update.properties
                    ...
                    query = f"""
                        ...
                        SET {set_clause}
                        WHERE id = ${idx}
                        RETURNING id, created_at, updated_at, email, first_name, last_name, is_active, is_superadmin, platform_id, properties
                    """
                    values.append(id)
            ...
            self.logger.error(f"{__name__}: Error updating admin {id} - {e}")
            raise e
    ...
    async def list(
    ...
    ) -> AsyncIterator[AdminRead]:
        """List admins with optional filters and JSONB property filters."""
        allowed_fields = {
            "id",
            "created_at",
            "updated_at",
        ...
        try:
            ...
            self.logger.error(f"{__name__}: Error listing admins - {e}")
            raise e
